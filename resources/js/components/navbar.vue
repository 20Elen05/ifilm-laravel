    ...
    <header class="container px-4" :class="{ 'night-mode': isNightMode }">
        <nav class="navbar pt-3 shadow-none navbar-light navbar-expand-lg">
            ...
            <select v-model="selectedLang"
                    class="font-light mx-2 text-light form-control border-0 p-0 shadow-none w-auto bg-transparent">
            ...
            <div class="navbar-collapse collapse" style="display: none;">
                <form class="d-flex ms-4 ms-sm-3 pt-lg-0 flex-nowrap justify-content-end form-inline">
                    <input
                        class="me-lg-2 radius-5 px-3 py-1 shadow-none bg-transparent form-control form-control-sm font-light"
                        v-model="searchedMovie" type="text" placeholder="Search" v-on:click="changeBorderColor">
                    <router-link v-if="searchedMovie" :to="{ name:'search', params: { keyword : this.searchedMovie}}">
                        <button name="search" size="sm" class="btn btn-if mx-2 my-sm-0 p-1 font-weight-bold size13">
                            Search
                        </button>
                    </router-link>
                </form>
                <div class="font-light navbar-nav px-3 font17 ms-auto test-right mt-2 mt-lg-0">
                    <router-link v-if="selectedLang === 'en'" :to="{ name:'movie', params:{ id: randomId }}"
                                 class="mx-3 ps-lg-3 text-light px-1">Random
                    </router-link>
                    <router-link v-if="selectedLang === 'ru'" :to="{ name:'movie', params:{ id: randomId }}"
                                 class="mx-3 ps-lg-3 text-light px-1">Случайны
                    </router-link>
                    <NightMode class="themeButton"></NightMode>
                    <label v-if="selectedLang === 'en'" for="themeMode" class="ms-3 custom-control-label">Night
                        Mode</label>
                    <label v-if="selectedLang === 'ru'" for="themeMode" class="ms-3 custom-control-label">Ночной
                        режим</label>
                </div>
            </div>
